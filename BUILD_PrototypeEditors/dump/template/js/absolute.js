...
window.norman = {
    services: {
        ...
        getRootControl: function () {
            //lazy loading
            if (!this._root) {
                var root = document.querySelector('.sapUiLayoutAbs') || {};
        ...
        controlAtPosition: function (x, y) {
            ...
            //only absolute layout and positioncontainers returned
            while (oCtrl && oCtrl !== this.getRootControl() && oCtrl.getParent() && oCtrl.getParent() !== this.getRootControl()) {
        ...
        moveControlToPosition: function (ctrl, x, y, offsetInView) {
            var parent = ctrl.getParent();
            if (parent !== this.getRootControl()) {
                ctrl.invalidate();
                return;
            }
            var rootOffset = parent.$().offset();
            ctrl.updatePosition({
                top: (y - offsetInView.top - rootOffset.top) + 'px',
                left: (x - offsetInView.left - rootOffset.left) + 'px'
            });
        },
